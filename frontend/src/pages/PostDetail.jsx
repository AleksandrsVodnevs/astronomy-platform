...
import { getPost, deletePost, createComment, deleteComment, getPosts } from '../services/api';
...
const PostDetail = () => {
  ...
  return (
    <div className={`pd-outer container${related.length > 0 ? ' pd-outer--with-sidebar' : ''}`}>
      ...
      {related.length > 0 && (
        <aside className="pd-sidebar">
          <h3 className="pd-sidebar-title">
            {lang === 'en' ? 'Related Posts' : 'Līdzīgi ieraksti'}
          </h3>
          {related.map((p) => (
            <Link to={`/forums/${p.id}`} key={p.id} className="pd-related-card">
              <span
                className="pd-related-cat"
                style={{ color: CAT_COLORS[p.category] || 'var(--accent)' }}
              >
                {translateCategory(p.category, lang)}
              </span>
              <h4 className="pd-related-title">{p.title}</h4>
